...
//    }
    ...
    public void handleLogout(MouseEvent event) {
        try {
            Scene scene = new Scene(FXMLLoader.load(getClass().getResource("Login.fxml")), 900, 600); // Đặt kích thước
            Stage stage = (Stage) logoutButton.getScene().getWindow();
            stage.setMaximized(false);
            stage.setScene(scene);
            stage.sizeToScene(); // Căn chỉnh theo nội dung
            stage.centerOnScreen(); // Đặt ở giữa màn hình
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
